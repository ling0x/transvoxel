...
//! 4. Interpolate vertex positions on active edges; compute normals via
...
//! 4. Look up the triangle pattern and interpolate vertices as above.
...
const EDGE_CORNERS: [(usize, usize); 12] = [
    (0, 1), (1, 3), (2, 3), (0, 2),  // bottom face
    (4, 5), (5, 7), (6, 7), (4, 6),  // top face
...
where
    F: Fn(f32, f32, f32) -> f32,
{
    // Skip boundary cells that will be handled by transition geometry
    if is_boundary_cell(ix, iy, iz, block.subdivisions, transitions) {
        return;
    }

    // 8 cell corners in grid coordinates
    ...
    // 8-bit case index
    let case_index = (0..8u8).fold(0u8, |acc, i| {
        if densities[i as usize] >= threshold { acc | (1 << i) } else { acc }
    }) as usize;
    ...
    if cell_data.len() < 1 + tri_count * 3 { return; }

    // Compute interpolated vertex positions along each active edge
    let mut vertex_buf: [u32; 12] = [0; 12];
    let mut vi = 0usize;

    for (edge_idx, &(ca, cb)) in EDGE_CORNERS.iter().enumerate() {
        if vi >= vert_count { break; }
        let da = densities[ca];
        let db = densities[cb];
        if (da >= threshold) == (db >= threshold) { continue; } // edge not crossed
        ...
        let normal = gradient_normal(field, block.voxel_step(), pos, field);
        vertex_buf[vi] = mesh.push_vertex(pos, normal);
        vi += 1;
        let _ = edge_idx; // suppress unused warning
    }

    // Emit triangles
    emit_triangles(cell_data, tri_count, vert_count, &vertex_buf, mesh);
}
...
// Edge index → (voxel_a, voxel_b) in the 3×3 grid (row-major: index = row*3 + col).
const TC_EDGE_CORNERS: [(usize, usize); 12] = [
    (0, 1), (1, 2),  // top row horizontal
    (3, 4), (4, 5),  // mid row horizontal
    (6, 7), (7, 8),  // bot row horizontal
    (0, 3), (1, 4), (2, 5),  // left/mid/right columns, upper half
    (3, 6), (4, 7), (5, 8),  // left/mid/right columns, lower half
];
    ...
    F: Fn(f32, f32, f32) -> f32,
{
    ...
    for cell_v in 0..n {
        for cell_u in 0..n {
            // 9 high-resolution sample positions on this transition cell face.
            // The 3×3 grid is twice the resolution of the normal voxel grid.
            let mut densities  = [0.0f32; 9];
            let mut positions  = [[0.0f32; 3]; 9];

            for row in 0..3usize {
                for col in 0..3usize {
                    // sub-grid coordinates within the transition cell
                    let sub_u = cell_u * 2 + col;
                    let sub_v = cell_v * 2 + row;

                    // Convert to block-level voxel indices (with sub-voxel interpolation)
                    let u_int = sub_u / 2;
                    let v_int = sub_v / 2;
                    ...
                    let sample_idx = row * 3 + col;
                    positions[sample_idx] = pos;
                    densities[sample_idx] = field(pos[0], pos[1], pos[2]);
                }
            }

            // 9-bit case index
            let case_index = (0..9usize).fold(0u16, |acc, i| {
            ...
            let tri_count  = ((cell_data[0] >> 4) & 0xF) as usize;
            let vert_count = (cell_data[0] & 0xF) as usize;
            if cell_data.len() < 1 + tri_count * 3 { continue; }

            // Compute interpolated vertices along active transition cell edges
            let mut vertex_buf: [u32; 12] = [0; 12];
            let mut vi = 0usize;

            for (_, &(ca, cb)) in TC_EDGE_CORNERS.iter().enumerate() {
                if vi >= vert_count { break; }
                let da = densities[ca];
                let db = densities[cb];
                ...
                let normal = gradient_normal(field, step, pos, field);
                vertex_buf[vi] = mesh.push_vertex(pos, normal);
                vi += 1;
            }
...
#[inline]
fn emit_triangles(cell_data: &[u8], tri_count: usize, vert_count: usize, verts: &[u32; 12], mesh: &mut Mesh) {
...
/// Central-difference gradient normal at `pos`.
#[inline]
fn gradient_normal<F>(field: &F, step: f32, pos: [f32; 3], _field2: &F) -> [f32; 3]
where
    F: Fn(f32, f32, f32) -> f32,
{
    let e = step * 0.1;
...
/// Returns `true` if the cell at `(ix, iy, iz)` lies on a face that has a transition side active.
/// Such cells are skipped in regular extraction and re-handled by the transition pass.
#[inline]
